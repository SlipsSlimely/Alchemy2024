/// @description This sets up all of the functions that run the battle system itself
...
{
	enemyUnits[i] = instance_create_depth(x+230+(i*10), y+110+(i*20), depth-10, oBattleUnitEnemy, enemies[i]);
...
//Shuffle turn order
unitTurnOrder = array_shuffle(units); //This currently randomizes the turn order, this will have to be adjusted later
	...
	if(!instance_exists(oMenu))
	{
		...
		if (_unit.object_index == oBattleUnitPC)
		{
			...
			for (var i = 0; i < array_length(_actionList); i++)
			{
				...
				var _available = true; // later we'll check mp cost or item count here.
				...
				for ( var i = 0; i < array_length(_subMenusArray); i ++)
				{
					...
					//add back option at then end of each submenu
...
function BeginAction(_user, _action, _targets)
{
	...
	with (_user)
	{
		...
		{
			sprite_index = sprites[$ _action.userAnimtion];
			image_index = 0;
		}
	}
