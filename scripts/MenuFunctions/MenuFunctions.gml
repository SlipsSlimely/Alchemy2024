...
function Menu(_x, _y, _options, _description = -1, _width = undefined, _height = undefined)
{
	with (instance_create_depth(_x, _y, -99999, oMenu))
	{
		...
		{
			width = 1;
			if(sDescription != -1) width = max(width, string_width(_description));
		...
		{
			height = heightLine * (_optionsCount + !(sDescription == -1));
		...
		else
		{
			heightFull = _height;
			//scrolling?
			if (heightLine * (_optionsCount + !(sDescription == -1)) > _height - (ymargin * 2))
			{
				scrolling = true;
